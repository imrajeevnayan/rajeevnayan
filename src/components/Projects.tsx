...
import { Github, ExternalLink, X, Target, Cpu, Lightbulb, HelpCircle, Key, Server, Globe } from 'lucide-react';
...
const Projects = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'problem' | 'workflow'>('overview');
              ...
              >
                ...
                <div className="p-6 md:p-8 overflow-y-auto space-y-6 flex-1">
                  ...
                  <div className="inline-flex gap-1 bg-[var(--surface-main)] p-1 rounded-full border border-[var(--border-main)]">
                    {(['overview', 'problem', 'workflow'] as const).map((tab) => (
                      <button
                        key={tab}
                      ...
                      >
                        {tab === 'overview' ? 'Overview' : tab === 'problem' ? 'Problem & Solution' : 'Architecture & Workflow'}
                      </button>
                    ))}
                  </div>

                  {/* Tab Content */}
                  <div className="min-h-[100px] text-sm text-[var(--text-secondary)] leading-relaxed font-light">
