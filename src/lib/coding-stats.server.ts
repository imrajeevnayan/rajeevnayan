// src/lib/coding-stats.server.ts
import { PlatformStats, CodingStats } from "./coding-stats.functions";
...
let lastManualRefresh: number = 0;
...
async function fetchPlatformStatsFromSVG(platform: "leetcode" | "gfg" | "codolio"): Promise<PlatformStats> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`Failed to fetch ${platform} SVG`);
...
export async function fetchAllStats(): Promise<CodingStats> {
...
export async function getCachedCodingStats(): Promise<CodingStats> {
  const now = Date.now();
...
export async function refreshCacheNow(): Promise<CodingStats> {
  const now = Date.now();
